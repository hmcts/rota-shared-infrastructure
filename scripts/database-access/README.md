...
1. Connect to the VPN via the [HMCTS Portal](https://portal.platform.hmcts.net/)
2. Run the relevant `paas-preview-password` script for your OS (Powershell or Bash) to display the preview database password
...
3. Run the relevant `hmcts-platform-keys` script for your OS (Powershell or Bash) to generate SSH keys that are required to connect to the bastion server
4. Run the relevant `paas-access-token` script for your OS (Powershell or Bash) to generate an access token for any of the databases
